...
class MSTParserLSTM:
    def __init__(self, new_options):
        ...
        if not new_options.predictFlag:
            vocab, w2i, pos, rels = utils.vocab(new_options.conll_train, new_options.conll_train_language)
            options = new_options
        else:
            with open(new_options.params, 'r') as paramsfp:
                vocab, w2i, pos, rels, ex_trn, exc_trnd, stored_opt = pickle.load(paramsfp)
                self.extrnd = ex_trn
            ...
            options = stored_opt
        print "     ls it multilingual?:", options.multilingual_train
        ...
        self.multilingual_train = options.multilingual_train
        ...
        self.dims = self.wdims + self.pdims + (self.landims if self.add_lang_vec else 0) + (self.edim if options.extConcateFlag else 0) + (self.ecdim if self.external_cluster_embedding else 0)
        ...
        if not new_options.predictFlag:
            ...
            with open(os.path.join(new_options.output, new_options.params), 'w') as paramsfp:
                    pickle.dump((vocab, w2i, pos, rels, self.extrnd, self.exctrnd, options), paramsfp)
        ...
        with open(conll_path, 'r') as conllFP:
            for iSentence, sentence in enumerate(read_conll(conllFP, self.conll_test_language)):
                ...
                for entry in conll_sentence:
                    ...
                    evec = None
                    ecevc = None
                    lang_code = self.languageVec_dic[entry.language].lang_code+':' if self.multilingual_train else ""
                    ...
                    entry.vec = concatenate(filter(None, [wordvec, posvec, evec, ecevc, langvec]))
